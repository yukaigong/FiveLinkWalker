...
% for i = 4:4
    ...
    pc_ini = Data_array.px_com(impacts_in_periodic(1));
    vc_ini = Data_array.vx_com(impacts_in_periodic(1));
    L_ini = Data_array.l_stToe(impacts_in_periodic(1));
    L_predict_01 = [L_ini];
    pc_predict_01 = [pc_ini];
    vc_predict_02 = [vc_ini];
    pc_predict_02 = [pc_ini];
    L_actual = Data_array.l_stToe(impacts_in_periodic(1):impacts_in_periodic(4));
    pc_actual = Data_array.px_com(impacts_in_periodic(1):impacts_in_periodic(4));
    vc_actual = Data_array.vx_com(impacts_in_periodic(1):impacts_in_periodic(4));
    t = Time_array(impacts_in_periodic(1):impacts_in_periodic(4));
    for j = 1:3
        ...
        p_stT_ini_step = Data_array.p_stT(1,impacts_in_periodic(j));
        rp_stT_ini_step = Data_array.rp_stT(1,impacts_in_periodic(j));
        for k = impacts_in_periodic(j)+1:impacts_in_periodic(j+1)
%             T = Time_array(impacts_in_periodic(2)) - Time_array(impacts_in_periodic(1));
            T = Time_array(k) - Time_array(impacts_in_periodic(j));
