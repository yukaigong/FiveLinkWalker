...
classdef FLW_Controller_Poincare <matlab.System & matlab.system.mixin.Propagates & matlab.system.mixin.SampleTime %#codegen
...
%             end
            obj.l_stToe_kf = L_stToe_obs(2);
            
            
            fp_ratio = obj.fp_ratio;
...
%             H = 0.6 + 0.2*min(1,t_total/10);
            ...
            dx0_next_dot = -l^2*cosh(l*T_left)*rp_stT(1) ...
                            + l*sinh(l*T_left)*rv_stT(1) ...
                            - l*sinh(l*T_left)*L_stToe(2)/(32*H)...
                            + cosh(l*T_left)*g/H*rp_stT(1);
            
            temp = dJrp_stT*dq; % a_com = J*ddq + dJ*dq; This is the second part
            dx0_next_ddot = l^3*sinh(l*T_left)*rp_stT(1) -l^2*cosh(l*T_left)*rv_stT(1) ...
                - l^2*cosh(l*T_left)*rv_stT(1) +  (l*sinh(l*T_left))*temp(1)... % Notice that the second part of a_com will be added to hr, (l*sinh(l*T_left))*ra_stT(1)) 
                + l^2*cosh(l*T_left)*L_stToe(2)/(32*H) - l*sinh(l*T_left)*g/H*rp_stT(1)...
                - l*sinh(l*T_left)*g/H*rp_stT(1) + cosh(l*T_left)*g/H*rv_stT(1);
            
%             dx0_next_dot = 0;
%             dx0_next_ddot = 0;
            
            x0_next_dot = (1-fp_ratio-cosh(l*obj.T))/(l*sinh(l*obj.T)) * dx0_next_dot;
            x0_next_ddot = (1-fp_ratio-cosh(l*obj.T))/(l*sinh(l*obj.T)) * dx0_next_ddot;
            ...
            ref_rv_swT_x = 1/2*(obj.rp_swT_ini(1) - x0_next)*(-w*sin(w*t))...
                            + (1/2 - 1/2*cos(w*t)) * x0_next_dot;
            ref_ra_swT_x = 1/2*(obj.rp_swT_ini(1) - x0_next) * (-w^2*cos(w*t))...
                            +1/2*w*sin(w*t) * x0_next_dot ...
                            + (1/2 - 1/2*cos(w*t))*x0_next_ddot...
                            + 1/2*w*sin(w*t)* x0_next_dot;
...
%             hr= [sqrt(dxf_next_goal)/5;H;ref_rp_swT_x;ref_rp_swT_z];
            ...
            % Jh_modification, dJh does not need to be modified.
            
            Jh_m = Jh - [zeros(1,7);...
                        zeros(1,7);...
                        (1/2 - 1/2*cos(w*t)) * (1-fp_ratio-cosh(l*obj.T))/(l*sinh(l*obj.T)) * (l*sinh(l*T_left))*Jrp_stT(1,:);...
                        zeros(1,7)];
%             Jh_m = Jh;
...
%             u = (Jh*S*Me^-1*Be)^-1*(-Kd*dy-Kp*y+ddhr+Jh*S*Me^-1*He);
            u = (Jh_m*S*Me^-1*Be)^-1*(-Kd*dy-Kp*y+ddhr+Jh_m*S*Me^-1*He - dJh*dq);
%             u = 10*ones(4,1)*sin(t);
%             u = zeros(4,1);
